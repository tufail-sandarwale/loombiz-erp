...
@Component({
  selector: 'app-quantity-udf',
  standalone: true,
  imports: [CommonModule, SharedMaterialModules, ReactiveFormsModule, TranslocoModule, ClientSideTableViewComponent],
  templateUrl: './quantity-udf.component.html',
  styleUrl: './quantity-udf.component.scss'
})
export class QuantityUdfComponent  implements OnInit{
  actions = [
  ...
  private loadPricingUDF(): void {
    this.udfService.getUDFByGroup(RVConstants.udfGroup.quantity).subscribe(result => {
