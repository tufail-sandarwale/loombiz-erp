...
@Component({
  selector: 'app-product-attribute-setting',
  standalone: true,
  imports: [CommonModule, SharedMaterialModules, ReactiveFormsModule, TranslocoModule, ClientSideTableViewComponent],
  templateUrl: './product-attribute-setting.component.html',
  styleUrl: './product-attribute-setting.component.scss'
})
export class ProductAttributeSettingComponent implements OnInit {
  ...
  loadSettings() {
    this.inventryService.getDefaultProductSettings('productAttributes').subscribe(
      (settings) => {
        const formGroup = {};
        ...
        settings.forEach((setting: any) => {
          ...
          formGroup[`id_${setting.code}`] = [{ value: setting.id, disabled: true }]; // Add 'id' control, disabled
        });
  ...
  saveSettings() {
    const updatedSettings = this.settingList.map((setting) => ({
      id: this.settingsForm.get(`id_${setting.code}`).value, // Retrieve the id value
      code: setting.code,
      defaultValue: this.settingsForm.value[setting.code],
    }));
